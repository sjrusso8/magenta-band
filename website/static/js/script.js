...
function init() {
    router.on("/solo", () => ready(true));
    router.on("/band", () => ready(false));
    
    fileBtn.addEventListener('change', loadFile);
    urlBtn.addEventListener('click', loadURL);
    saveBtn.addEventListener('click', () => saveAs(new File([mm.sequenceProtoToMidi(currentSample)], 'aiband_.mid')));
...
async function showInput(ns) {
    ...
    if (isMelodyMode && shouldSplit) {
        ...
        vizMelody = new mm.PianoRollSVGVisualizer(
            melody,
            document.getElementById('vizMelody'), { noteRGB: '35,70,90', activeNoteRGB: '157, 229, 184', noteHeight: 3 });
        updateUI('has-melody');
    } else {
    ...
    vizInput = new mm.PianoRollSVGVisualizer(
        input,
        document.getElementById('vizInput'), { noteRGB: '35,70,90', activeNoteRGB: '157, 229, 184', noteHeight: 3 });
...
function updateVisualizer() {
    vizSample = new mm.PianoRollSVGVisualizer(
        currentSample, document.getElementById('vizSample'), { noteRGB: '35,70,90', activeNoteRGB: '157, 229, 184', noteHeight: 5 });
}
...
function plotLoss(data) {
    const svg = d3.select('#trainingGraph');
    ...
    group.append("text")
        ...
        .text("Model Loss on Training Data");
...
function updateUI(state) {
    ...
    switch (state) {
        ...
        case 'file-loading':
            ...
            hide('input');
            hide('hasMelody');
            document.getElementById('status').textContent = 'Reading the file';
            break;
        case 'file-error':
            show('status');
            document.getElementById('status').textContent = 'Make sure it\'s a valid MIDI and try again?';
            break;
        case 'file-loaded':
            hide('status');
            show('section_3');
        ...
        case 'training-done':
            ...
            enable('section_1');
            console.log("beforeTraining")
            hide('beforeTraining');
            console.log("doneTraining")
            show('doneTraining');
            console.log("btnTrain")
            disable('btnTrain');
            btnSample.focus();
...
function updateCopy() {
    modeText.textContent = isMelodyMode ? 'Going Solo' : 'Joined the Band';
